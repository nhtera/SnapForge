...
sed "s/MARKETING_VERSION: \"${CURRENT_VERSION}\"/MARKETING_VERSION: \"${NEW_VERSION}\"/" "$PROJECT_FILE" > "${PROJECT_FILE}.tmp" && mv "${PROJECT_FILE}.tmp" "$PROJECT_FILE"

# Bump build number
CURRENT_BUILD=$(grep -m1 'CURRENT_PROJECT_VERSION:' "$PROJECT_FILE" | sed 's/.*: *"//' | sed 's/".*//')
NEW_BUILD=$((CURRENT_BUILD + 1))
sed "s/CURRENT_PROJECT_VERSION: \"${CURRENT_BUILD}\"/CURRENT_PROJECT_VERSION: \"${NEW_BUILD}\"/" "$PROJECT_FILE" > "${PROJECT_FILE}.tmp" && mv "${PROJECT_FILE}.tmp" "$PROJECT_FILE"
